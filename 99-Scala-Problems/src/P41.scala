...
object P41 {
  ...
  def printGoldbachListLimited(r: Range, limit: Int): Unit = {
    (r filter { n => n > 2 && n % 2 == 0 } map { n => (n, P40.goldbach(n)) }
      filter {
      _._2._1 >= limit
    } foreach {
      _ match {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(s"The list of all even numbers and the Goldbach composition in the range $r is: ")
    printGoldbachList(r)
    ...
    println(s"The list of all even numbers and the Goldbach composition in the range $r is: ")
    printGoldbachListLimited(r, limit)
